...
function setActiveOnScroll(){
		var index = $('.panelize .panel.scroll').last().index();
		if ( index >=0  )
		{
			var className = 'screen-' + ( index +1 );
			$('div#header').attr('class',"").addClass(className)
			$('div#header > nav > a').eq( index ).addClass('active').siblings().removeClass('active');
		}
}